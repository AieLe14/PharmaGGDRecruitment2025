...
const CartHeader = () => {
  ...
  // Vérifier que le composant est hydraté côté client
  useEffect(() => {
    setIsHydrated(true)
  }, [])
  ...
  // Pendant l'hydratation, afficher toujours le modal pour éviter les différences SSR/client
  if (!isHydrated) {
    return <CartModal isOpen={isCartOpen} onClose={handleCloseCart} />
  }

  // Ne pas afficher le header du panier s'il n'y a pas d'articles
  if (totalItems === 0) {
    return <CartModal isOpen={isCartOpen} onClose={handleCloseCart} />
  }
